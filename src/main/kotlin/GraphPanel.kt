...
class GraphPanel(private  var scores: List<Double>) : JPanel() {
    ...
    override fun paintComponent(g: Graphics) {
        ...
        val yScale = (getHeight().toDouble() - 2 * padding - labelPadding) / (maxScore - minScore)
        ...
        for (i in 0 until numberYDivisions + 1) {
            ...
            val y0 =
                getHeight() - (i * (getHeight() - padding * 2 - labelPadding) / numberYDivisions + padding + labelPadding)
            if (scores.size > 0) {
                g2.color = gridColor
                g2.drawLine(padding + labelPadding + 1 + pointWidth, y0, getWidth() - padding, y0)
                g2.color = Color.BLACK
                val yLabel =
                    (((minScore + (maxScore - minScore) * (i * 1.0 / numberYDivisions)) * 100).toInt() / 100.0).toString() + ""
        ...
        for (i in scores.indices) {
            if (scores.size > 1) {
                val x0 = i * (getWidth() - padding * 2 - labelPadding) / (scores.size - 1) + padding + labelPadding
                val y0 = getHeight() - padding - labelPadding
